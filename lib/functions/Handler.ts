        ...
        try {
            ...
            if (msg.noPrefix == '>') {
                try {
                    ...
                    let text = (format(await eval(`(async() => { ${q} })()`))).replace(Config.MONGGO_URI, 'uri gw anjg').replace(undefined, 'undepined<😅☝🏻>')
                ...
                if (!msg.isOwner) return;
                try {
                    let text = (format(await eval(`(async() => { ${transpile(q)} })()`))).replace(Config.MONGGO_URI, 'uri gw anjg')
